...
suite('Unit Tests', function(){
  ...
  suite('Equality', function() {
    ...
    test('#equal, #notEqual', function(){
      ...
      assert.equal( 6 * '2', '12', 'integer 12 == string "12"');
      assert.notEqual( 6 + '2', '12', '8 is not == to "12"');
    });
    ...
    test('#strictEqual, #notStrictEqual', function(){
      ...
      assert.notStrictEqual([1, 'a', {}], [1, 'a', {}], 'arrays are not strict equals even is they have the same elements');
    });
...
// -----------------------------------------------------------------------------
  ...
  suite('Comparisons', function() {
    ...
    test('#isAbove, #isAtMost', function() {
      ...
      assert.isAtMost(1 - Math.random(), 1, "1 - rand <= 1");
    });

    /** 9 - .isBelow() => a < b , .isAtLeast =>  a >= b **/
    test('#isBelow, #isAtLeast', function() {
      assert.isAtLeast('world'.length , 5, "5 >= 5");
      assert.isAtLeast(2*Math.random(), 0, "2 * rand >= 0");
      assert.isBelow(5 % 2, 2, "1 < 2");
      assert.isBelow(2/3, 1, "0 < 1");
    });
